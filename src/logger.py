...
def save_trade(record) -> int:
    ...
    cur.execute("""
        INSERT INTO trades
        (timestamp, action, coin, price, amount, value_usd, balance_after, pnl, reasoning, confidence, risk_level)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        ...
        record.confidence, record.risk_level,
    ))
    conn.commit()
...
def save_snapshot(market_data: dict):
    ...
    cur.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        market_data.get("collected_at", datetime.utcnow().isoformat()),
